...
import { Card, Col, Row, Button, Table, Dropdown, Toast } from "react-bootstrap";
                      ...
                      <Row>
                        ...
                        <Col xl={3}>
                          <Dropdown>
                            ...
                            <Dropdown.Toggle
                              variant={myData.status === "REJECTED" ? "danger" : "secondary"}
                              id="dropdown-basic"
                            >
                              {getStatusOptions(myData.type).find((el) => (reduceString(el.value) === reduceString(myData.status)))?.label || myData.status}
                            </Dropdown.Toggle>
                            <Dropdown.Menu>
                              {getStatusOptions(myData.type).map((statusOption, index) => (
                                <Dropdown.Item
                                  key={index}
                                  onClick={() => handleOrderStatus(myData.id, statusOption.value)}
                                >
                                  {statusOption.label}
                                </Dropdown.Item>
                              ))}
